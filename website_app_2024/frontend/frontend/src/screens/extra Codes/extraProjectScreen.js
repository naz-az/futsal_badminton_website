...
function ProjectScreen() {
  ...
  const greyButtonStyle = { backgroundColor: "grey" };
  const greenButtonStyle = { backgroundColor: "green" };
  const redButtonStyle = { backgroundColor: "red" };
  ...
  const [vote, setVote] = useState(null);  // This will store the user's current vote

  useEffect(() => {
    async function fetchProject() {
      const { data } = await axios.get(`/api/projects/${id}`);
      setProject(data);
      setUpvotes(data.upvotes); // <-- Update upvotes state here
      setDownvotes(data.downvotes); // <-- Update downvotes state here
      setUserVote(data.user_vote); // <-- Set user's current vote
    }
...
const handleVote = async (voteType) => {
  ...
  try {
    ...
    const response = await axios.post(`/api/projects/${id}/vote/`, { vote: voteType }, config);

    const { upvotes, downvotes, userVote } = response.data;
    setUpvotes(upvotes);
    setDownvotes(downvotes);
    setUserVote(userVote); // Immediately reflect the new vote in the UI
    console.log("Updated vote counts - Upvotes:", upvotes, "Downvotes:", downvotes);
  } catch (error) {
