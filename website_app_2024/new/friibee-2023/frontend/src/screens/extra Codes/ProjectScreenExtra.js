...
function ProjectScreen() {
  ...
  const [vote, setVote] = useState(0);  // 1 for upvote, -1 for downvote, 0 for no vote
  ...
  useEffect(() => {
    async function fetchProject() {
      ...
      setVote(data.user_vote); // Set the initial vote state based on the fetched user vote
      console.log("Fetched vote:", data.user_vote); // Log the fetched vote
...
const handleVote = async (newVote) => {
  ...
  try {
    await axios.post(`/api/projects/${id}/vote/`, { vote: newVote }, config);
    setVote(newVote);
    // Fetch updated project data
    const { data } = await axios.get(`/api/projects/${id}`);
    setProject(data.project); // Update to handle the new response structure
  } catch (error) {
...
return (
  ...
  <Button
    variant={vote === 1 ? "success" : "secondary"}
    onClick={() => handleVote(vote === 1 ? 0 : 1)}
  >
    Thumbs Up
  </Button>
  <span>{project.upvotes - project.downvotes}</span>
  <Button
    variant={vote === -1 ? "danger" : "secondary"}
    onClick={() => handleVote(vote === -1 ? 0 : -1)}
  >
    Thumbs Down
  </Button>
