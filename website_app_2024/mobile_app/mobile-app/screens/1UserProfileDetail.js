...
import { View, Text, Button, ScrollView, Image, TouchableOpacity, Picker, StyleSheet } from "react-native";
...
function UserProfileDetail() {
  ...
  useEffect(() => {
    sortProjects('date', selectedSort);
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'center', marginTop: 10 }}>
          {profile.social_facebook && (
            <TouchableOpacity onPress={() => Linking.openURL(profile.social_facebook)}>
              <FontAwesome5 name="facebook" size={24} color="blue" style={{ marginHorizontal: 10 }} />
            </TouchableOpacity>
          )}
  
          {profile.social_twitter && (
            <TouchableOpacity onPress={() => Linking.openURL(profile.social_twitter)}>
              <FontAwesome5 name="twitter" size={24} color="#1DA1F2" style={{ marginHorizontal: 10 }} />
            </TouchableOpacity>
          )}
  
          {profile.social_instagram && (
            <TouchableOpacity onPress={() => Linking.openURL(profile.social_instagram)}>
              <FontAwesome5 name="instagram" size={24} color="#C13584" style={{ marginHorizontal: 10 }} />
            </TouchableOpacity>
          )}
  
          {profile.social_youtube && (
            <TouchableOpacity onPress={() => Linking.openURL(profile.social_youtube)}>
              <FontAwesome5 name="youtube" size={24} color="red" style={{ marginHorizontal: 10 }} />
            </TouchableOpacity>
          )}
  
          {profile.social_website && (
            <TouchableOpacity onPress={() => Linking.openURL(profile.social_website)}>
              <FontAwesome5 name="globe" size={24} color="black" style={{ marginHorizontal: 10 }} />
            </TouchableOpacity>
          )}
        </View>
      </View>
  ...
  onPress={async () => {
    if (!await isAuthenticated()) {
      ...
      navigation.navigate('SendMessage', { recipient: id });
    }
  }}
  style={styles.button}
>
  <Text style={styles.buttonText}>Send Message</Text>
    ...
    <View style={{ flexDirection: 'row', justifyContent: 'space-evenly', marginTop: 20 }}>
      <TouchableOpacity onPress={() => navigation.navigate('Followers', { id: id })}>
        <Text style={{ textAlign: 'center' }}>
          <Text style={{ fontWeight: 'bold' }}>{profile.followers_count}</Text> Followers
        </Text>
      </TouchableOpacity>
      <TouchableOpacity onPress={() => navigation.navigate('Following', { id: id })}>
        <Text style={{ textAlign: 'center' }}>
    ...
    <View style={styles.card}>
      ...
      <View style={styles.cardBody}>
        ...
        >
          ...
          <Picker.Item label="High to Low (Top)" value="topToLow" />
          <Picker.Item label="Low to High (Top)" value="lowToTop" />
              ...
              <View key={project.id}>
                ...
                <View style={{ flexDirection: 'row', marginTop: 10, flexWrap: 'wrap' }}>
                  {project.tags.map((tag) => (
                    <TouchableOpacity key={tag.id} onPress={() => navigation.navigate('Category', { tagId: tag.id })}
                ...
                <Text style={{ fontSize: 16, marginTop: 10 }}>
                  <Text style={styles.brandBadge}>{project.brand}</Text>
                </Text>
