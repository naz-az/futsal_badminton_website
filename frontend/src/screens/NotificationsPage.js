...
const NotificationsPage = () => {
  ...
  const renderNotificationItem = (notif) => {
    ...
    switch (notif.notification_type) {
      case "FOLLOWER":
        const profileLink =
          notif.follower.id === auth.user.profile.id
            ? "/user/account"
            : `/profiles/${notif.follower.id}`;
      ...
      case "COMMENT":
        const commentProfileLink =
          notif.comment.user.id === auth.user.profile.id
            ? "/user/account"
            : `/profiles/${notif.comment.user.id}`;
            ...
            <div className="d-flex justify-content-between">
              <div className="d-flex align-items-center">
                ...
                <small style={{ marginLeft: "5px" }}>
                  commented <strong>"{notif.comment.content}"</strong> on your
                  project:{" "}
                </small>
              </div>
              <div className="d-flex align-items-center">
                <Link
                  to={`/project/${notif.comment.project.id}`}
      ...
      case "VOTE":
        const voteProfileLink =
          notif.voting_user.id === auth.user.profile.id
            ? "/user/account"
            : `/profiles/${notif.voting_user.id}`;
          ...
          <div className="d-flex flex-column">
            <div className="d-flex justify-content-between align-items-center">
              <div className="d-flex align-items-center">
                ...
                <small style={{ marginLeft: "5px" }}>
                  has liked your project:
                </small>
              </div>
              <div className="d-flex align-items-center">
                <Link
                  to={`/project/${notif.project.id}`}
                  className="no-underline"
                >
            ...
            <div className="d-flex justify-content-between">
              <div className="d-flex align-items-center">
                ...
                <span style={{ marginRight: "5px" }}>
                  <strong>"{notif.comment.content}"</strong>
                </span>
                <small style={{ marginRight: "5px" }}>on your comment</small>
                <span style={{ marginRight: "5px" }}>
                  <strong>"{notif.replied_comment.content}"</strong>
                </span>{" "}
                on project:
              </div>
              <div className="d-flex align-items-center">
  ...
  const followerNotifications = notifications.filter(
    (notif) => notif.notification_type === "FOLLOWER"
  );
  const commentNotifications = notifications.filter(
    (notif) => notif.notification_type === "COMMENT"
  );
  const voteNotifications = notifications.filter(
    (notif) => notif.notification_type === "VOTE"
  );
          ...
          <Card>
            ...
            <ListGroup variant="flush">
              ...
              {notifications.filter(
                (notif) => notif.notification_type === "REPLY"
              ).length > 0 ? (
                notifications
                  .filter((notif) => notif.notification_type === "REPLY")
                  .map((notif) => (
                    <ListGroup.Item
                      key={notif.id}
                      action
